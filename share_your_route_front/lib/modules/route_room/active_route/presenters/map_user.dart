...
class MapUserPage extends StatelessWidget {
  @override
...
class MapUser extends StatefulWidget {
  ...
  @override
  State<MapUser> createState() => MapUserState();
}

class MapUserState extends State<MapUser> with WidgetsBindingObserver {
  ...
  void initState() {
   ...
   Future<void> _checkValorEnServidor() async {
    try {
      var response= await getRouteById("${activeTouristRouteDic['id']}");
       print("${activeTouristRouteDic['id']}");
      TouristRoute route = getRouteWithId(response!)['route'];
      print(route.name);
      if (route.hasStarted == true) {
        setState(() {
          routeStarted= true;
          getNewRouteFromAPI();
        });
      } else {
        print('Error al obtener el valor del servidor');
      }
    } catch (e) {
      print('Error: $e');
    }
       ...
       Positioned(
          ...
          Positioned.fill(
            child:Container(
                ...
                child: Center(child: Container( alignment: Alignment.center, child:
                Text("Esperando a que el guía inicie la ruta..." , textAlign: TextAlign.center,style: TextStyle(color: Colors.white, fontSize: 20,)))))
              )
          ,],
           Positioned(
            bottom: 30,
